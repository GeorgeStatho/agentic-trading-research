...
class DashboardOpenPositionsTests(unittest.TestCase):
    def test_option_rows_recompute_pl_from_quote_midpoint_when_broker_pl_is_stale(self):
        raw_positions = [
            {
        ...
        expected_unrealized_pl_pct = round(((expected_mid_price - 3.25) / 3.25) * 100.0, 4)
        ...
        self.assertNotEqual(position["unrealized_pl_pct"], round(0.7846 * 100.0, 4))
