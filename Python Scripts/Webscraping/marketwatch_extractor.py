...
# These selectors are based on the saved MarketWatch search snapshot in:
# `Stock-trading-experiment/html_pages/Search - MarketWatch.htm`
# That page shows result cards built around:
# - `.element.element--article`
# - `.article__headline a.link`
# - `.article__timestamp`
# - `.article__author`
# The live article page HTML is not saved in the repo, so the article-body
# selectors below are a conservative mix of MarketWatch-specific containers and
# generic article fallbacks.
...
def extract_marketwatch_search_links(response: Response) -> list[dict]:
    ...
    fallback_links = response.css("a[href*='/story/']")
