...
def _empty_trend_quality_payload(*, error: str = "") -> TrendQualityPayload:
    if error:
        # Keep a stable payload shape even when Sharpe inputs are unavailable.
        _ = error
    return {
...
def _build_company_price_context(
    ...
    # Assemble strategist-side price context once so history and trend-quality metrics stay aligned.
